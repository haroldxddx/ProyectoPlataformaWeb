...
namespace ProyectoPlataformaW.Datos
{
    public class clProfesorD
    {
        ...
        public List<clProfesorEEE> mtdListarProf()
        {
            ...
            public List<clProfesorEEE> mtdBuscarProf(clProfesorEEE objProfe)
            {
                string sql = "select Nombres,Apellidos,Documento,Email,Especialidad from Profesor WHERE (Profesor.Nombres LIKE '%" + objProfe.Nombres + "%')";
                clAdminSQL objSql = new clAdminSQL();
                DataTable tblprof = new DataTable();
                tblprof = objSql.mtdDesconectado(sql);
                ...
                for (int i = 0; i < tblprof.Rows.Count; i++)
                {
                clProfesorEEE objProfee = new clProfesorEEE();

                    
                    objProfee.Nombres = tblprof.Rows[i][0].ToString();
                    objProfee.Apellidos = tblprof.Rows[i][1].ToString();
                    objProfee.Documento = int.Parse(tblprof.Rows[i][2].ToString());
                    objProfee.Email = tblprof.Rows[i][3].ToString();
                    objProfee.Especialidad = tblprof.Rows[i][4].ToString();
